...
const NotesPanel: React.FC<NotesPanelProps> = ({ isOpen, onClose, notes, onDelete, onUpdate }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex flex-col sm:flex-row">
      <div className="w-full sm:w-64 md:w-80 bg-slate-900 border-r border-slate-800 flex flex-col">
              ...
              {notes.map(note => (
                <li key={note.id}>
                  <a
                    ...
                    className={`block p-3 rounded-md truncate transition-colors text-left ${activeNoteId === note.id ? 'bg-cyan-900/50 text-white' : 'text-slate-300 hover:bg-slate-800'}`}
                  >
      ...
      <main className="flex-1 flex flex-col">
        <header className="flex justify-end items-center p-4 bg-slate-900/80 backdrop-blur-sm border-b border-slate-800">
          {activeNote && (
            <button
                onClick={() => {
        ...
        <div className="flex-grow p-6 overflow-y-auto">
          {activeNote ? (
            <div>
              ...
              <div className="text-sm text-slate-400 mb-4 pb-4 border-b border-slate-700">
              ...
              <textarea
                ...
                onBlur={handleUpdate}
                className="w-full h-[60vh] p-4 bg-slate-800/50 border border-slate-700 rounded-md focus:ring-2 focus:ring-cyan-500 focus:outline-none transition duration-200 resize-y text-slate-300"
